...
function App() {
  ...
  const courseColors = [
    ...
    '#34495E', '#E67E22', '#2ECC71', '#C0652A', '#8B7EC8'
  ];

  const handleAddCourse = (message) => {
    // Simple parsing logic - you can enhance this later with AI
    const courseCodeMatch = message.match(/(\d{5})\s*([A-Z]?)/i);
    
    if (courseCodeMatch) {
      const [, code, section] = courseCodeMatch;
      const fullCode = section ? `${code} ${section}` : code;
      
      // Check if course already exists
      if (courses.find(c => c.code === fullCode)) {
        return false;
      }

      // Create a new course (you'll add real course data later)
      const newCourse = {
        id: Date.now(),
        code: fullCode,
        name: 'New Course',
        ...
        schedule: [
          { day: 0, startTime: 9, duration: 1.5 },
          { day: 2, startTime: 9, duration: 1.5 },
        ]
      };
          ...
          <>
            ...
            <div className="content-area">
              <div className="chat-section">
                <ChatArea onAddCourse={handleAddCourse} />
              </div>
